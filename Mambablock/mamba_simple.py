...
def test_mamba():
    ...
    input_w = 32
    input_c = 32
    seq_len = input_h * input_w * input_c
    ...
    model = Mamba(
        ...
        input_w=input_w,
        input_c=input_c,
        bimamba_type="v2"  # You can test with "v2" as well
    ).to(device)

    # Create random input data
    hidden_states = torch.randn(batch_size, seq_len, d_model).to(device)
    extra_emb = torch.randn(batch_size, seq_len, d_model).to(device)
    alpha = torch.ones(12).to(device)

    # Forward pass
    output, outputs = model(hidden_states, extra_emb, alpha=alpha)
